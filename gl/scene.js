...
import { config, VIDEO_SOURCES } from "./config.js";
...
export function createCarousel(canvas, { onActiveChange } = {}) {
  ...
  function coverRatio(video) {
    const cardAspect = config.cardWidth / config.cardHeight;
    const videoAspect = video.videoWidth / video.videoHeight || cardAspect;
    return new THREE.Vector2(
      Math.min(1, cardAspect / videoAspect),
      Math.min(1, videoAspect / cardAspect)
    );
  }
  ...
  VIDEO_SOURCES.forEach((src, index) => {
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uProgress: { value: 0 },
        uIndex: { value: index },
        uCount: { value: VIDEO_SOURCES.length },
    ...
    mesh.userData.entry = config.entry ? 1 : 0;
    ...
    video.addEventListener("loadeddata", () => {
      ...
      material.uniforms.uImageRatio.value.copy(coverRatio(video));
      video.play().catch(() => {});
      textureSettled();
    });
  ...
  function focusCard(index) {
    ...
    const base = index - total / 2;
    const nearest =
      base + Math.round((scroll.state.current - base) / total) * total;
    scroll.goTo(nearest);
  }
  ...
  function tick() {
    ...
    if (config.autoSpin) scroll.state.target += config.autoSpin;
